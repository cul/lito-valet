RSpec.describe 'Barnard Remote Request Service' do
  ...
  it 'Barnard Remote form submission renders confirm and sends email' do
    user = FactoryBot.create(:happyuser)
    sign_in user
    params = { id: '5060332', itemBarcodes: ['0036014419'] }
    post barnard_remote_index_path, params: params
    ...
    # staff email
    expect(staff_email.from).to include(APP_CONFIG[:barnard_remote][:staff_email])
    expect(staff_email.to).to include(APP_CONFIG[:barnard_remote][:staff_email])
    ...
    # confirm email
    expect(confirm_email.from).to include(APP_CONFIG[:barnard_remote][:staff_email])
  ...
  it 'rejects Barnard Remote Requests for non-Barnard Remote items' do
    sign_in FactoryBot.create(:happyuser)
    get barnard_remote_path('123')
  ...
  it 'rejects Barnard Remote Requests for Partner ReCAP items' do
    sign_in FactoryBot.create(:happyuser)
    get barnard_remote_path('SCSB-1441991')
    expect(response.body).to include('record has no Barnard Remote holdings')
  end
