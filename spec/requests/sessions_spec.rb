...
# CAS_HOST (cas.columbia.edu, hardcoded to match omniauth-cul's ColumbiaCas
# strategy) is allowed to leave the host - Rails 7's raise_on_open_redirects
# (via config.load_defaults 7.0) blocks cross-host redirect_to by default.
# This is what makes /sign_out able to bounce the browser through CAS to kill
# the CAS session, not just Valet's local session - see
...
RSpec.describe 'Sessions' do
  ...
  describe 'GET /sign_out' do
    it 'allows the cross-host redirect to CAS so the CAS session is also killed' do
      ...
      expect(response).to redirect_to(%r{\Ahttps://cas\.columbia\.edu/cas/logout\?service=})
    end
